...
void RangeSensorLayer::onInitialize()
{
  ...
  // Addition by dimkirt
  std::string pandora_range_topic_name;

  nh.param("pandora_range_topic", pandora_range_topic_name, std::string());
  ...
  for (unsigned int i = 0; i < topic_names.size(); i++)
  {
    ...
    else
    {
      ...
      // (dimkirt) overwrite sonar topic
      topic_name = pandora_range_topic_name;
